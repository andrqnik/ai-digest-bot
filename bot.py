...
async def chat_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    system_prompt = (
        ...
        "РЕЖИМЫ ОТВЕТА:\n\n"
        "РЕЖИМ 1 — ФАКТЫ (используй по умолчанию):\n"
        "Отвечай только фактами из дайджеста. Не добавляй свои оценки, выводы и рассуждения. "
        "Если факты из дайджеста найдены — в самом конце ответа добавь одну строку: "
        "«Хочешь моё мнение по этой теме?» "
        "Если информации в дайджесте нет — так и скажи: «В дайджесте эта информация отсутствует.»\n\n"
        "РЕЖИМ 2 — МНЕНИЕ (только если пользователь явно попросил: «что думаешь», «посоветуй», "
        "«как считаешь», «хочу твоё мнение», «да» в ответ на твоё предложение высказаться):\n"
        "Дай развёрнутое личное мнение по конкретному вопросу из этого сообщения. "
        "Мнение даётся только по теме текущего вопроса — не переноси его на другие темы.\n\n"
        "КОНТЕКСТ ДИАЛОГА:\n"
        "Всегда учитывай историю переписки. Короткий вопрос («а если я хочу X?», «а что насчёт Y?») "
        "— это продолжение предыдущей темы, а не новый запрос. Не сбрасывай контекст.\n\n"
        "ИСТОЧНИКИ:\n"
        "К каждому факту прикладывай источник из дайджеста. "
        "Если есть URL — давай его полностью. Если URL нет — указывай название источника курсивом.\n\n"
        "ФОРМАТИРОВАНИЕ (строго):\n"
        "Используй только HTML-теги Telegram: <b>жирный</b>, <i>курсив</i>. "
        "Запрещено использовать ** ## • и любые другие markdown-символы."
        + digest_context
    )
